# /pkgs — Send to Packages session

## Direct Communication (tmux send-keys)

If arguments are provided, inject the message directly into the Packages tmux pane:

```bash
PANE_ID=$(tmux list-panes -a -F '#{pane_id} #{pane_title}' | grep -i 'pkgs\|packages' | grep -v 'CURSOR' | head -1 | awk '{print $1}')
if [ -n "$PANE_ID" ]; then
  tmux send-keys -t "$PANE_ID" "$ARGUMENTS" Enter
  echo "Sent to Packages ($PANE_ID): $ARGUMENTS"
else
  echo "ERROR: Packages pane not found. Is the swarm running?"
