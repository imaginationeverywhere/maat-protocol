...
### Step 1b: Detect Team Identity and Auto-Rename (CRITICAL)
...
### Step 8: Family Standup (NON-NEGOTIABLE)
...
### Step 9: Load Operating Rules (NON-NEGOTIABLE)
...
### Step 13: Join the Swarm Network (NON-NEGOTIABLE)
...
**4. Start the 5-Minute Feed Cron (NON-NEGOTIABLE):**
Every session MUST create a CronCreate job that checks the live feed every 5 minutes and surfaces team updates directly in the conversation. The feed watcher (tail) catches events instantly but only writes to a file — the cron brings updates INTO the conversation where the user and agents can see them.

```
CronCreate:
  cron: "*/5 * * * *"
  prompt: "Check the swarm live feed for team updates. Read the last 20 lines of ~/auset-brain/Swarms/live-feed.md. Report any AGENDA COMPLETE, REPORTING IN, PROGRESS, or TO:<team-name> messages that are new since last check. Keep it brief — team name and what they said. If nothing new, say nothing."
```

**Both the tail AND the cron must run.** Tail = instant file detection. Cron = surfaces updates in the conversation. They complement each other.
