...
/git-sweep --yes                    # Apply changes
/git-sweep --dry-run                # Explicit preview
/git-sweep --merged-only            # Only touch branches fully merged into develop
/git-sweep --remote                 # Also delete matching origin branches
/git-sweep --worktrees              # Worktree cleanup only (skip branch deletion)
...
- **Confirms** before applying if >20 operations (unless `--yes`)
...
| **Stale** | Not merged AND last commit > `--age` days ago | Listed for review; not deleted automatically (use `--merged-only=false` + `--yes` to sweep) |
