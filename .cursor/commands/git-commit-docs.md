...
This command accepts optional flags that modify its behavior:

**Usage:** `/git-commit-docs [--no-verify] [--no-push] [--amend] [--force-push] [custom message]`
...
- `--force-push` - Use `git push --force` (use with caution!)
- Custom text at the end will be appended to the commit message body

**Examples:**
- `/git-commit-docs` - Full workflow with docs, commit, and push
- `/git-commit-docs --no-push` - Update docs and commit only (no push)
- `/git-commit-docs --no-verify` - Skip pre-commit hooks
- `/git-commit-docs --no-verify --no-push` - Local commit without hooks or push
- `/git-commit-docs --amend` - Amend previous commit with new changes
- `/git-commit-docs Added emergency hotfix` - Append custom note to commit

**Argument Parsing:**
When the user provides arguments, extract them at the start of the workflow:
```typescript
const args = userInput.split(' ').slice(1); // Everything after /git-commit-docs
const flags = {
...
8. **Update MVP Playground Data (Auto-Regenerate)**
   - Run: `node scripts/generate-playground-data.js`
   - This regenerates `frontend/public/mvp-playground-data.json` from codebase analysis
   - Stage the updated file: `git add frontend/public/mvp-playground-data.json`
   - Skip silently if `scripts/generate-playground-data.js` does not exist

9. **Update Root CHANGELOG.md (REQUIRED - MUST BE LAST)**
...
10. **Execute Git Commands (COMPREHENSIVE STAGING)**
   ...
   **Parse command arguments first:**
   - Check if user provided `--no-verify`, `--no-push`, `--amend`, or `--force-push` flags
   - Extract any custom message text
   ...
   **Commit with appropriate flags:**
   ...
   # With --amend and --no-verify
   git commit --amend --no-verify -m "<generated message>"
   ```
   ...
   **Push to remote (unless --no-push specified):**
   ...
   # With --force-push flag (CAUTION!)
   git push --force origin <current-branch>

   # Skip if --no-push flag provided
   # (do not execute push command)
   ```

   **Append custom message if provided:**
   - If user provided custom text (not starting with --), append to commit message body
   - Example: `/git-commit-docs This is urgent` → adds "This is urgent" to commit body
