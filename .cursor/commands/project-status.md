...
1. **`docs/status/MASTER_TASKS.md`** - PRIMARY SOURCE: Task definitions, completion status, agent assignments
...
const mvpTasks = parseMasterTasks('docs/status/MASTER_TASKS.md');
...
When `project-status` runs, it generates/updates these files in `docs/status/`:
...
Plan Location:         {project-path}/docs/status/plans/roadmap-plan.md
```
...
2. **Review Generated Plans**:
   - Roadmap tasks: `docs/status/ROADMAP_TASKS.md`
   - Quarterly plan: `docs/status/QUARTERLY_PLAN.md`
   - Execution plan: `docs/status/plans/roadmap-plan.md`

3. **Execute with Auto-Claude**:
   - Auto-Claude opens the project directory
   - Reads `docs/status/plans/roadmap-plan.md`
