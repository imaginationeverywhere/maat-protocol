# /trackit — Send to TrackIt session

## Direct Communication (tmux send-keys)

If arguments are provided, inject the message directly into the TrackIt tmux pane:

```bash
PANE_ID=$(tmux list-panes -a -F '#{pane_id} #{pane_title}' | grep -i 'trackit' | grep -v 'CURSOR' | head -1 | awk '{print $1}')
if [ -n "$PANE_ID" ]; then
  tmux send-keys -t "$PANE_ID" "$ARGUMENTS" Enter
  echo "Sent to TrackIt ($PANE_ID): $ARGUMENTS"
else
  echo "ERROR: TrackIt pane not found. Is the swarm running?"
...
```bash
.claude/scripts/session-registry.sh wake "TrackIt" "$ARGUMENTS"
```

3. **Confirm** with status: "Sent to TrackIt. [Active on TTY X / NOT FOUND]"
