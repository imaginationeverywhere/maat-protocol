# /qcr — Send to QCR session

## Direct Communication (tmux send-keys)

If arguments are provided, inject the message directly into the QCR tmux pane:

```bash
PANE_ID=$(tmux list-panes -a -F '#{pane_id} #{pane_title}' | grep -i 'qcr' | grep -v 'CURSOR' | head -1 | awk '{print $1}')
if [ -n "$PANE_ID" ]; then
  tmux send-keys -t "$PANE_ID" "$ARGUMENTS" Enter
  echo "Sent to QCR ($PANE_ID): $ARGUMENTS"
else
  echo "ERROR: QCR pane not found. Is the swarm running?"
...
```bash
.claude/scripts/session-registry.sh wake "QCR" "$ARGUMENTS"
```

3. **Confirm** with status: "Sent to QCR. [Active on TTY X / NOT FOUND]"
