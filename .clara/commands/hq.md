# /hq — Send to HQ session

## Direct Communication (tmux send-keys)

If arguments are provided, inject the message directly into the HQ tmux pane:

```bash
PANE_ID=$(tmux list-panes -a -F '#{pane_id} #{pane_title}' | grep -i 'hq\|headquarters' | grep -v 'CURSOR' | head -1 | awk '{print $1}')
if [ -n "$PANE_ID" ]; then
  tmux send-keys -t "$PANE_ID" "$ARGUMENTS" Enter
  echo "Sent to HQ ($PANE_ID): $ARGUMENTS"
else
  echo "ERROR: HQ pane not found. Is the swarm running?"
...
```bash
.claude/scripts/session-registry.sh wake "Headquarters" "$ARGUMENTS"
```

3. **Confirm** with status: "Sent to HQ. [Active on TTY X / NOT FOUND]"
