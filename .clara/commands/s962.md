# /s962 — Send to Site 962 session

## Direct Communication (tmux send-keys)

If arguments are provided, inject the message directly into the Site 962 tmux pane:

```bash
PANE_ID=$(tmux list-panes -a -F '#{pane_id} #{pane_title}' | grep -i 's962\|site.*962\|962' | grep -v 'CURSOR' | head -1 | awk '{print $1}')
if [ -n "$PANE_ID" ]; then
  tmux send-keys -t "$PANE_ID" "$ARGUMENTS" Enter
  echo "Sent to Site 962 ($PANE_ID): $ARGUMENTS"
else
  echo "ERROR: Site 962 pane not found. Is the swarm running?"
...
```bash
.claude/scripts/session-registry.sh wake "962" "$ARGUMENTS"
```

3. **Confirm** with status: "Sent to Site 962. [Active on TTY X / NOT FOUND]"
