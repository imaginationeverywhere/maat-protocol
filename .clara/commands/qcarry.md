# /qcarry — Send to QuikCarry session

## Direct Communication (tmux send-keys)

If arguments are provided, inject the message directly into the QuikCarry tmux pane:

```bash
PANE_ID=$(tmux list-panes -a -F '#{pane_id} #{pane_title}' | grep -i 'qcarry\|carry' | grep -v 'CURSOR' | head -1 | awk '{print $1}')
if [ -n "$PANE_ID" ]; then
  tmux send-keys -t "$PANE_ID" "$ARGUMENTS" Enter
  echo "Sent to QuikCarry ($PANE_ID): $ARGUMENTS"
else
  echo "ERROR: QuikCarry pane not found. Is the swarm running?"
...
```bash
.claude/scripts/session-registry.sh wake "Carry" "$ARGUMENTS"
```

3. **Confirm** with status: "Sent to QuikCarry. [Active on TTY X / NOT FOUND]"
